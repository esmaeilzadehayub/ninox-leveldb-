# ──────────────────────────────────────────────────────────────────
# IAM — IRSA roles for Velero and Loki (no static AWS credentials)
# Uses: terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks
# ──────────────────────────────────────────────────────────────────
...
module "velero_irsa" {
  ...
  # Inline policy for S3 + EC2 snapshot permissions
  role_policy_arns = {
...
resource "aws_iam_policy" "velero" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      # ── S3: read/write the backup bucket ─────────────────
      {
        Sid    = "VeleroS3Objects"
        Effect = "Allow"
        Action = [
      ...
      # ── EC2: EBS snapshots (for EBS-backed PVCs if any) ──
      {
        Sid    = "VeleroEC2"
        Effect = "Allow"
        Action = [
...
resource "aws_iam_policy" "loki" {
  ...
  description = "Loki S3 log storage permissions"
...
# ════════════════════════════════════════════════════════════════════
# GITHUB ACTIONS IRSA (CI/CD — ECR push + EKS deploy)
# ════════════════════════════════════════════════════════════════════
    ...
    rules = [
      {
        ...
        selection    = { tagStatus = "tagged", tagPrefixList = ["v"], countType = "imageCountMoreThan", countNumber = 20 }
        action       = { type = "expire" }
      },
      {
        rulePriority = 2
